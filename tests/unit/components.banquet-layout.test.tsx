...
vi.mock('@/components/ui/calendar', () => ({
  Calendar: ({ onSelect }: any) => {
    ...
    return React.createElement('div', {
      ...
      onClick: () => onSelect(new Date('2025-12-10')),
    });
  },
}));
  ...
  ToggleGroup: ({ children, onValueChange }: any) => {
    return React.createElement(
      'div',
      {
        onClick: (e: any) => {
          const btn =
            (e.target as HTMLElement).closest && (e.target as HTMLElement).closest('button');
          if (btn) {
...
vi.mock('@/context/reservation-context', () => ({
  useReservations: () => ({ addReservation: mockAddReservation }),
}));
...
const mockTables = [
  {
    ...
    state: true,
    available: true,
    availableSeats: [{ id: 11, seatNumber: 1, available: true, reservationId: '' }],
  },
];
