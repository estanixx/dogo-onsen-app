...
vi.mock('@/components/shared/time-slot-selector', () => ({ __esModule: true, default: ({ availableTimeSlots, onSelect }: any) => React.createElement('div', null, availableTimeSlots.map((s: string) => React.createElement('button', { key: s, onClick: () => onSelect(s) }, s))) }));
vi.mock('@/components/ui/dialog', () => ({ Dialog: ({ children }: any) => React.createElement('div', null, children), DialogContent: ({ children }: any) => React.createElement('div', null, children), DialogHeader: ({ children }: any) => React.createElement('div', null, children), DialogFooter: ({ children }: any) => React.createElement('div', null, children), DialogTitle: ({ children }: any) => React.createElement('div', null, children), DialogDescription: ({ children }: any) => React.createElement('div', null, children), DialogTrigger: ({ children }: any) => React.createElement('div', null, children) }));
vi.mock('@/components/ui/scroll-area', () => ({ ScrollArea: ({ children }: any) => React.createElement('div', null, children) }));

vi.mock('@/lib/api', () => ({ getAvailableTimeSlotsForService: vi.fn(async () => ['09:00 AM']), createServiceReservation: vi.fn(async () => ({ id: 'r1' })) }));
...
describe('ServiceBookConfirm', () => {
  ...
  it('loads time slots and allows confirming a reservation', async () => {
    ...
    render(<ServiceBookConfirm service={service} open={true} setOpen={() => {}} account={account} />);

    const api = await import('@/lib/api');
    await waitFor(() => expect(api.getAvailableTimeSlotsForService).toHaveBeenCalledWith(service.id, expect.any(Date)));
