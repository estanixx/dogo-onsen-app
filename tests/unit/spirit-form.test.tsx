...
vi.mock('@/components/ui/label', () => ({ Label: (props: any) => React.createElement('label', { ...props }, props.children) }));
...
vi.mock('@/components/ui/select', () => {
  ...
  const Select = ({ children, value, onValueChange }: any) => React.createElement(Ctx.Provider, { value: { onValueChange, value } }, children);
...
vi.mock('./camera-capture', () => ({ __esModule: true, default: ({ onCapture, onUploadComplete }: any) => React.createElement('div', { 'data-testid': 'mock-camera' }, 'camera') }));
...
// Mock spirit context
vi.mock('@/context/spirit-context', () => ({ useSpirit: () => ({ spirits: [], setSpirits: () => {} }) }));

// Mock API
const mockGetAllTypes = vi.fn(async () => [{ id: 't1', name: 'Type1' }]);
const mockGetSpirit = vi.fn(async (id: number) => null);
const mockCreateSpirit = vi.fn(async (id: number, name: string, typeId: string, image: string) => ({ id: id, name }));
vi.mock('@/lib/api', () => ({ getAllSpiritTypes: () => mockGetAllTypes(), getSpirit: (...args: any[]) => mockGetSpirit(...args), createSpirit: (...args: any[]) => mockCreateSpirit(...args) }));
